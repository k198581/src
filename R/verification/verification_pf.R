...
VerificationPF <- function(file, dir, ext=NULL) {
  
  for (pen in -1) {
    # Set the path of the matching rate.
    ansrate.file <- paste("../../Alignment/", file, "_", ext, "_", pen, "_", format(Sys.Date()), ".txt", sep = "")
    
    # Set the path of the PSA directory.
    output.dir <- paste("../../Alignment/", dir, "_", ext, "_", pen, "_", format(Sys.Date()), "/", sep = "")
